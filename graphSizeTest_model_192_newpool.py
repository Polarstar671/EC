...
class cgcnn(base_model):
    ...
    def _inference(self, x): 
        ...
        list=[i/192 for i in range(1,193)] 
        tList=tf.reshape(list, [1,192,1,1]) 
        tList = tf.tile(tList, multiples=[batch, 1, M, 1])
        x=tf.multiply(x,tList)
        ...
        [x_mean0, x_varia0] = tf.nn.moments(x0, axes=[0, 1])  
        
        x0 = tf.nn.batch_normalization(x0, x_mean0, x_varia0, offset, scale, vari_epsl)

        
        x0=tf.reduce_sum(x0,1, keep_dims=True)
        batch, N, M, C = x0.get_shape() 
        
        x = tf.reshape(x0, [batch, N*M * C])
