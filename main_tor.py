...
from data import load_data, load_data2, load_data3, split_dataset, DataGenerator
import tor_lstm
import tor_sdae
import tor_cnn
...
def failRawData(rawData, failNum, noiseType, flowSizeList, zeroCode=int(1514 / 32)):
    ...
    if noiseType == 0:
        ...
        for i in range(sampleSize):
            ...
            sampLocations = random.sample(curFlowIdx, int(failNum * curFlowSize))  # 随机抽取乱序的位置集合
            AList = sampLocations.copy()  # 1, 3, 2
            sampLocations.sort()  # 1, 2, 3
            rawData[i][sampLocations] = rawData[i][AList]  # 对应位置互换
    return rawData
...
def main(save=False, wtime=False):
    ...
    data, labels, flowSizeList = load_data3(datapath,
                             minlen=minlen,
                             maxlen=maxlen,
                             traces=traces,
                             dnn_type=dnn)
