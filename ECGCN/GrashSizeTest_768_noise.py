#-*-coding:utf-8-*-
import MySQLdb
from lib import graphSizeTest_model_768_noise, graph, coarsening, utils
import numpy as np
...
def loadTrafficData(packetNumsPerFlow=200, regions=80, originA=None, types=30, zeroCode=int(1514/4)):
    conn = MySQLdb.connect("localhost", "root", "******", "*******", charset='utf8' )
    cur=conn.cursor()
    flowSizeList=[]
    
    sql1='select (pkt_length*flow_seq+1514)/4, round(time,6), FLOW_ID  from ALL_PACKETS_1000 order by FLOW_ID, PKT_ID asc'
    cur.execute(sql1)
    infos = cur.fetchall()
    rawList=[]
...
def calSimilarity(C=40, packetNumsPerFlow=1000):
    conn = MySQLdb.connect("localhost", "root", "Dzl900425", "networkTraffic", charset='utf8' )
    cur=conn.cursor()
...
def runModel(filterFunctionName, isDAE=1):
    ...
    X=failRawData(X,0.2,0,flowSizeList)
    ...
    model = graphSizeTest_model_768_noise.cgcnn(**params)
