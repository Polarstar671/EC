...
from lib import graphSizeTest_model_192, graph, coarsening, utils
import numpy as np
...
def loadTrafficData(packetNumsPerFlow=200, regions=80, originA=None, types=30, zeroCode=int(1514/16)):
    ...
    sql1='select (pkt_length*flow_seq+1514)/16, round(time,6), FLOW_ID  from ALL_PACKETS_1000 order by FLOW_ID, PKT_ID asc' #0-79
    cur.execute(sql1)
    infos = cur.fetchall()
    rawList=[]
...
def runModel(filterFunctionName, isDAE=1):
    ...
    params['eval_frequency'] = int(X_train.shape[0] / params['batch_size']) #200
    # Building blocks.
    ...
    model = graphSizeTest_model_192.cgcnn(**params)
