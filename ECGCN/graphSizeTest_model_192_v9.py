...
class cgcnn(base_model):
    ...
    def gcnPooling_V7(self, X_g, A_g, idx_g, scope, pooling_ratio=0.8):
        batch, N, M, C = X_g.get_shape()
        ...
        e = tf.nn.leaky_relu(tf.matmul(a_s, res), alpha=0.5)
    ...
    def gcnPooling_V8(self, X_g, A_g, scope, pooling_ratio=0.8):
        batch, N, M, C = X_g.get_shape()
        ...
        d = tf.matrix_diag(d)
        proj=tf.eye(int(N), batch_shape=[batch])+tf.matmul(d, tf.eye(int(N), batch_shape=[batch])-A_g)
        p_score=tf.matmul(proj,X_g)
                
        N_new = int((int(N) * pooling_ratio))
        attW_r = self._definedTuckerWeight_variable('attentionW_r', [1, M*C, N_new], scope, regularization=False)
        attW_r = tf.tile(attW_r, multiples=[batch, 1, 1])


        S_assign = tf.nn.softmax(tf.matmul(p_score, attW_r))
    ...
    def _inference(self, x): 
        ...
        temporal4Kernel=self._definedTuckerWeight_variable('xInput4Kernel', [1, 3, 8, 16], scope, regularization=False)
        x=tf.nn.conv2d(x, 
                        temporal4Kernel,
                        strides=[1,1,1,1], 
                        padding='SAME')
        ...
        temporal5Kernel=self._definedTuckerWeight_variable('xInput5Kernel', [1, 3, 16, 32], scope, regularization=False)
        x=tf.nn.conv2d(x, 
                        temporal5Kernel,
                        strides=[1,1,1,1], 
                        padding='SAME')
        ...
        print('x0:',x0)
        x0 = self.fc(x0, 120, 'timeFC', relu=False)
        x0=tf.reshape(x0,[batch, N, 1, 120])
        print('1 gcnconv')

        K=3
        A_diag0=self.redefine_A_6(x0, new_A, 'param1')
        spatialKernel1 = self._definedTuckerWeight_variable('gcnKernel1',[K, 120, 120], ('encodeDecoderConv{}').format(1), regularization=False)
        x1=self.gonv_new(x0, A_diag0, K, spatialKernel1)
        x1,A_diag1= self.gcnPooling_V9(x1, A_diag0, ('gcnPooling{}').format(1), 0.6)
        
        print('2 gcnconv',x1)
        A_diag2=self.redefine_A_6(x1, A_diag1, 'param2')

        spatialKernel2 = self._definedTuckerWeight_variable('gcnKernel2',[K, 120, 120], ('encodeDecoderConv{}').format(2), regularization=False)
        x2=self.gonv_new(x1, A_diag2, K, spatialKernel2)
        x2, A_diag2=self.gcnPooling_V9(x2, A_diag2, ('gcnPooling{}').format(2), 0.6)
        
        print('3 gcnconv',x2)
        A_diag3=self.redefine_A_6(x2, A_diag2, 'param3')
        spatialKernel3 = self._definedTuckerWeight_variable('gcnKernel3',[K, 120, 120], ('encodeDecoderConv{}').format(3), regularization=False)
        x3=self.gonv_new(x2, A_diag3, K, spatialKernel3)
        x3, A_diag3=self.gcnPooling_V9(x3, A_diag3, ('gcnPooling{}').format(3), 0.6)
  
        r=tf.concat([x0,x1,x2,x3], axis=1)  
        print('r:',r)
        batch, N, M, C = r.get_shape()

        x = tf.reshape(r, [batch, N*M * C])
