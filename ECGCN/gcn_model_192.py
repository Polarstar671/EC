...
class cgcnn(base_model):
    ...
    def gcnPooling_V6(self, X_g, A_g, idx_g, scope, pooling_ratio = 0.8):
    ...
    def gcnPooling_V8(self, X_g, A_g, idx_g, scope, pooling_ratio=0.8):
        ...
        X_g = tf.reshape(X_g, [batch, N, M * C])
        X = tf.matmul(X_g, tf.transpose(X_g,[0,2,1]))
        ...
        R=tf.matmul(W,tf.nn.relu(X))+bs
        S=tf.nn.softmax(R)+0.5
        merges=[]
    ...
    def onehotLayer(self, x, region):
        ...
        x_inside=tf.tile(x_inside,[1,1,192])
    ...
    def _inference(self, x): 
        ...
        x=tf.one_hot(xInput, 379)
        print('.....x:',x)
        
        offsetInput=tf.reshape(inputs[1],[batch,M,1,1])
        offset=tf.tile(offsetInput,[1,1,1,379])
