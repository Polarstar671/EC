...
class base_model(object):
    ...
    def fit(self, train_data, train_labels, val_data, val_labels):
        ...
        for step in range(1, num_steps + 1):
            ...
            if step % self.eval_frequency == 0 or step == num_steps:
                ...
                string, loss, predictions = self.evaluate(val_data, val_labels, sess, 15)
    ...
    def _get_session(self, sess=None):
        ...
        if sess is None:
            ...
            self.op_saver.restore(sess, filename) #restore all the variables读取训练好的模型参数
        return sess
...
class cgcnn(base_model):
    ...
    def redefine_A_2(self, x, A, idx=None):
        ...
        row1_squaresum=1/tf.sqrt(tf.clip_by_value(tf.reduce_sum((1/int(M-1))*tf.square(x1_mean), axis=2, keep_dims=True), 0.00000001,10000000)) #batchxNx1
        
        row2_mean=tf.reduce_mean(x_t_mid, axis=2, keep_dims=True)
        x2_mean=x_t_mid-row2_mean

        row2_squaresum_u=1/tf.sqrt(tf.clip_by_value(tf.reduce_sum((1/int(M-1))*tf.square(x2_mean), axis=2, keep_dims=True), 0.00000001,10000000)) #batchxNx1   1/miu
    ...
    def _inference(self, x): 
        ...
        x=tf.one_hot(xInput, 758)
        print('.....x:',x)
        
        offsetInput=tf.reshape(inputs[1],[batch,M,1,1])
        offset=tf.tile(offsetInput,[1,1,1,758])
