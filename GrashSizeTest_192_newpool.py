...
from lib import graphSizeTest_model_192_newpool, graph, coarsening, utils
import numpy as np
...
def normalMatrix(ajacencyMatrix, C):
    degree = np.zeros((C,C))
    rowsum = ajacencyMatrix.sum(axis=1)
    rowsum = np.clip(rowsum, 0.00000001,10000000)
    for j in range(C):
        degree[j,j] = 1/rowsum[j]
        
    ajacencyMatrix=degree*ajacencyMatrix 
...
def runModel(filterFunctionName, isDAE=1):
    ...
    model = graphSizeTest_model_192_newpool.cgcnn(**params)
