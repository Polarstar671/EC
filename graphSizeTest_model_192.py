...
class cgcnn(base_model):
    ...
    def _inference(self, x): 
        ...
        eye_tensor=tf.eye(int(N),batch_shape=[batch],dtype=tf.float32)
        oe_tensor=one_tensor-eye_tensor
        oe_tensor=tf.multiply(new_A,oe_tensor)
        new_A=tf.multiply(new_A,oe_tensor)
        ...
        x0 = self.fc(x0, 60, 'timeFC2', relu=True)
        print('timeFC x0:',x0)  
        ...
        x1,A_diag0, idx_seq1=self.gcnPooling_V6(x1, A_diag0, graph_idx, ('gcnPooling{}').format(1), 0.6)
        
        print('2 gcnconv',x1)
        A_diag0=self.redefine_A_6(x1, A_diag0, 'param2')
       
        spatialKernel2 = self._definedTuckerWeight_variable('gcnKernel2',[K, 60, 60], ('encodeDecoderConv{}').format(2), regularization=False)
        x2=self.gonv_new(x1, A_diag0, K, spatialKernel2)    
        x2, A_diag0, idx_seq2=self.gcnPooling_V6(x2, A_diag0, idx_seq1, ('gcnPooling{}').format(2), 0.6)

        
        print('3 gcnconv',x2)
        A_diag0=self.redefine_A_6(x2, A_diag0, 'param3')
        spatialKernel3 = self._definedTuckerWeight_variable('gcnKernel3',[K, 60, 60], ('encodeDecoderConv{}').format(3), regularization=False)
        x3=self.gonv_new(x2, A_diag0, K, spatialKernel3)    
        x3, A_diag0, idx_seq3=self.gcnPooling_V6(x3, A_diag0, idx_seq2, ('gcnPooling{}').format(3), 0.6)
        
        batch, N, M, C = x0.get_shape()
        x0=tf.reshape(x0,[batch, N*M*C])
        
        batch, N, M, C = x3.get_shape()
        x3=tf.reshape(x3,[batch, N*M*C])
                
       
        
        [x_mean0, x_varia0] = tf.nn.moments(x0, axes=[0,1])
        [x_mean1, x_varia1] = tf.nn.moments(x1, axes=[0,1])
        [x_mean2, x_varia2] = tf.nn.moments(x2, axes=[0,1])
        [x_mean3, x_varia3] = tf.nn.moments(x3, axes=[0,1])
