...
import sklearn, scipy.sparse, numpy as np, os, time, collections, shutil
from scipy import linalg
import scipy.sparse as sp
#from tensorflow.compat.v1 import batch_gather
...
class base_model(object):
    ...
    def predict(self, data, labels=None, sess=None, types=12):
        loss = 0
        size = data.shape[0]
    ...
    def evaluate(self, data, labels, sess=None, types=12):
    ...
    def fit(self, train_data, train_labels, val_data, val_labels, types=12):
        t_process, t_wall = time.process_time(), time.time()
        ...
        for step in range(1, num_steps + 1):
            ...
            print('steps:'+str(step)+';num_steps:'+str(num_steps)+';loss:'+str(loss))#+';params:'+str(params)+';flops:'+str(flops)+';timeconsume:'+str(timeconsume))
            ...
            if step % self.eval_frequency == 0 or step == num_steps:
                ...
                string, loss, predictions = self.evaluate(train_data, train_labels, sess, types)

                rightNum = 0
                for temp_idx in range(train_labels.shape[0]):
                    if np.argmax(predictions[temp_idx]) == train_labels[temp_idx]:
                        rightNum = rightNum + 1
                print('训练分类准确率为:%.2f', rightNum / train_labels.shape[0])

                string, loss, predictions = self.evaluate(val_data, val_labels, sess, types)

                rightNum=0
                for temp_idx in range(val_labels.shape[0]):
                    if np.argmax(predictions[temp_idx])==val_labels[temp_idx]:
                        rightNum=rightNum+1
                print('验证分类准确率为:%.2f',rightNum/val_labels.shape[0])
    ...
    def build_graph(self, M_0=45, inputDepth=200, types=12, channel=1):
      
        self.graph = tf.Graph()
        with self.graph.as_default():
            with tf.name_scope('inputs'):
                self.ph_data = tf.placeholder(tf.float32, (self.batch_size, inputDepth, 2), name='ph_data')
                self.ph_labels = tf.placeholder(tf.int32, (self.batch_size), name='ph_labels')
...
#Forward operation: filter_7/conv2d_transpose_1.  Input index: 2. Original input shape: (25, 45, 10, 16).  
#Calculated input gradient shape: (25, 23, 20, 16)
#这里反卷积后的输出是(25, 45, 20, 2)。但是报错，感觉本应该在temporal方向的反卷积在spatial方向上执行了。 
    ...
    def gcnPooling(self, X, A, idx, scope, pooling_ratio = 0.8):#如何处理channel的问题
    ...
    def redefine_A_4(self, x, A, name, idx=None):
        ...
        param1 = self._definedTuckerWeight_variable(name+'_1',[M*C, 20], 'redefineA', regularization=False)
        param2 = self._definedTuckerWeight_variable(name+'_2',[M*C, 20], 'redefineA', regularization=False)
        ...
        row1_mean=tf.reduce_mean(x_mid, axis=2, keep_dims=True)
        x1_mean=x_mid-row1_mean
        row1_squaresum=1/tf.sqrt(tf.clip_by_value(tf.reduce_sum(tf.square(x1_mean), axis=2, keep_dims=True), 0.00000001,10000000))
        
        row2_mean=tf.reduce_mean(x_t_mid, axis=2, keep_dims=True)
        x2_mean=x_t_mid-row2_mean
      
        row2_squaresum_u=1/tf.sqrt(tf.clip_by_value(tf.reduce_sum(tf.square(x2_mean), axis=2, keep_dims=True), 0.00000001,10000000))
        row2_squaresum=tf.transpose(row2_squaresum_u,[0,2,1])
    ...
    def redefine_A_6(self, x, A, name, idx=None):
        ...
        originParam = self._definedTuckerWeight_variable(name,[1, 1, M*C], 'redefine', regularization=False)
        param = tf.tile(originParam, multiples=[batch, N, 1])
        ...
        A_r = tf.matmul(A, x_mid)
        print('A_r:', A_r)
        
      
        similar=tf.sqrt(tf.maximum(tf.abs(tf.matmul(A_r,A_l)), 1e-10))
        ...
        new_A = tf.nn.relu(similar)
        new_A=tf.abs(new_A)
        A_degree=tf.clip_by_value(tf.reduce_sum(new_A,axis=2), 1,10000000)
        d = 1 / A_degree
        d=tf.matrix_diag(d)
        new_A=tf.matmul(d, new_A)
    ...
    def redefine_A_7(self, x, A, name, idx=None):
        ...
        originParam = self._definedTuckerWeight_variable(name,[1, M*C, M*C], 'redefine', regularization=False)
        param = tf.tile(originParam, multiples=[batch, 1, 1])
        
        B_l=tf.matmul(x,param)
        B_r=tf.transpose(B_l,[0,2,1])
        B=tf.nn.relu(tf.matmul(B_l,B_r))
      
        
      
        basicExp1=tf.matmul(tf.matmul(A,B),A)
        
        new_A=A-basicExp1+tf.matmul(tf.matmul(basicExp1,B),A)
        new_A=tf.abs(tf.multiply(new_A,oe_tensor))
        ...
        eye_tensor=tf.eye(int(N),batch_shape=[batch],dtype=tf.float32)
        new_A=eye_tensor-new_A
        return new_A

    def redefine_A_8(self, x, A, name, idx=None):
        ...
        x_mid=tf.multiply(x, param)
        x_t=tf.transpose(x, [0,2,1])
        B=tf.matmul(x_mid,x_t)
        
        similar=tf.matmul(tf.matmul(A,B),A)
      
        similar=tf.multiply(similar,oe_tensor)

        new_A=A-tf.nn.relu(similar)
        new_A=tf.abs(new_A)
        A_degree=tf.clip_by_value(tf.reduce_sum(new_A,axis=2), 1,10000000)
        d = 1 / A_degree
        d=tf.matrix_diag(d)
    ...
    def gcnPooling_V2(self, X, A, idx, scope, pooling_ratio = 0.8):#如何处理channel的问题
        batch, N, M, C = X.get_shape()
        X=tf.reshape(X, [batch*N, M*C])
    ...
    def gcnPooling_V3(self, X, A, idx, scope, pooling_ratio = 0.8):#如何处理channel的问题
        batch, N, M, C = X.get_shape()
        X=tf.reshape(X, [batch, N, M*C])
    ...
    def gcnPooling_V4(self, X, A, idx, scope, pooling_ratio = 0.8):#如何处理channel的问题
        batch, N, M, C = X.get_shape()
        X=tf.reshape(X, [batch, N, M*C])
    ...
    def gcnPooling_V5(self, X, A, idx, scope, pooling_ratio = 0.8):#如何处理channel的问题
        batch, N, M, C = X.get_shape()
        X=tf.reshape(X, [batch, N, M*C])
    ...
    def gcnPooling_V6(self, X_g, A_g, idx_g, scope, pooling_ratio = 0.8):#如何处理channel的问题
        batch, N, M, C = X_g.get_shape()
        X_g=tf.reshape(X_g, [batch, N, M*C])
        ...
        A_degree=tf.clip_by_value(tf.reduce_sum(tf.eye(int(N), batch_shape=[batch])-A_g,axis=2), 0.00000001,10000000)
      
        d = 1 / A_degree
        d = tf.matrix_diag(d)
      
        proj=tf.eye(int(N), batch_shape=[batch])-tf.matmul(d, tf.eye(int(N), batch_shape=[batch])-A_g) 
     
        p_score=tf.reduce_sum(tf.abs(tf.matmul(proj,X_g)),axis=2)
        value, topk_idx = tf.nn.top_k(p_score, to_keep)
        ...
        X_stack = tf.batch_gather(X_g, retainIdx)
        mid_A=tf.transpose(tf.batch_gather(A_g, retainIdx), [0,2,1])
        A_diag = tf.batch_gather(mid_A,  retainIdx)
        idx_seq = tf.batch_gather(idx_g, retainIdx)
    ...
    def gcnPooling_V7(self, X_g, A_g, scope, pooling_ratio=0.8):
        batch, N, M, C = X_g.get_shape()
      
        X_g = tf.reshape(X_g, [batch, N, M * C])
        X_old = tf.identity(X_g)#batch,N,M*C
        X_g = tf.matmul(A_g, X_g)
        X_g = tf.transpose(X_g, [0, 2, 1])

        N_new = int((int(N) * pooling_ratio))

        attW_r = self._definedTuckerWeight_variable('attentionW_r', [1, N, N_new], scope, regularization=False)
        attWs_r = tf.tile(attW_r, multiples=[batch, 1, 1])

        res_r = tf.matmul(X_g, attWs_r)

        res_l = tf.matmul(X_g, attWs_r)

        res = tf.matmul(tf.transpose(res_l, [0, 2, 1]), res_r)
        a = self._definedTuckerWeight_variable('attentionA', [1, N_new, N_new], scope, regularization=False)
        a_s = tf.tile(a, multiples=[batch, 1, 1])
        e = tf.nn.leaky_relu(tf.matmul(a_s, tf.transpose(res,[0,2,1])), alpha=0.25)
        S_normalized = tf.nn.softmax(e, dim=1)
        
        X_new = tf.matmul(tf.matmul(tf.transpose(X_old,[0,2,1]), attWs_r), S_normalized)
        X_new=tf.transpose(X_new,[0,2,1])
        X_new=tf.reshape(X_new,[batch, N_new, M, C])
        
        A_new = tf.matmul(A_g,attWs_r)
        A_new = tf.matmul(tf.transpose(attWs_r,[0,2,1]), A_new)
        
      
        return X_new,A_new

    def gcnPooling_V13(self, X_g, A_g, scope, pooling_ratio=0.8):
        batch, N, M, C = X_g.get_shape()
        ...
        S=tf.matmul(attWs_r, S_normalized)
        
      
        X_new = tf.matmul(tf.transpose(attWs_r,[0,2,1]), X_old)
        X_new=tf.reshape(X_new,[batch, N_new, M, C])
        
        A_new = tf.matmul(tf.matmul(tf.transpose(attWs_r,[0,2,1]), A_g), attWs_r)
        return X_new,A_new

    def gcnPooling_V14(self, X_g, A_g, scope, pooling_ratio=0.8):
        batch, N, M, C = X_g.get_shape()
        ...
        X_new = tf.matmul(tf.transpose(S,[0,2,1]), X_old)
        X_new=tf.reshape(X_new,[batch, N_new, M, C])
        
        A_new = tf.matmul(tf.matmul(tf.transpose(S,[0,2,1]), A_g), S)
        return X_new,A_new    
    def gcnPooling_V9(self, X_g, A_g, scope, pooling_ratio=0.8):
        batch, N, M, C = X_g.get_shape()
      
        N_new = int((int(N) * pooling_ratio))
        
        K=3
        spatialKernel1 = self._definedTuckerWeight_variable('poolKernel',[K, C, N_new], scope, regularization=False)
        p_score=self.gonv_new(X_g, A_g, K, spatialKernel1)
        p_score = tf.reshape(p_score, [batch, N, N_new])

        S_assign = tf.nn.leaky_relu(p_score)
        S_assign = tf.nn.softmax(p_score)
        S_assign_T= tf.transpose(S_assign, [0,2,1])
        
        X_g = tf.reshape(X_g, [batch, N, M * C])
        X_new = tf.matmul(S_assign_T, X_g)
        X_new = tf.reshape(X_new, [batch, N_new, M, C])
        A_new = tf.matmul(tf.matmul(S_assign_T,A_g),S_assign)
        return X_new,A_new
    
    
    def gcnPooling_v10(self, X_g, A_g, scope, pooling_ratio=0.8):
        batch, N, M, C = X_g.get_shape()
        N_new = int((int(N) * pooling_ratio))        
        ...
        p_score = tf.matmul(A_g, X_g)
        p_score = tf.transpose(p_score, [0, 2, 1])
        res_r = tf.matmul(p_score,attWs_r)
        res_l = tf.identity(res_r)
        res = tf.matmul(tf.transpose(res_l, [0, 2, 1]), res_r)
        
        a = self._definedTuckerWeight_variable('attentionA', [1, N_new, N_new], scope, regularization=False)
        a_s = tf.tile(a, multiples=[batch, 1, 1])
        e = tf.nn.leaky_relu(tf.matmul(a_s, res), alpha=0.25)
      
        S=tf.matmul(attWs_r,e)
        S=tf.nn.softmax(S, dim=2)
        
        X_new = tf.matmul(tf.transpose(S,[0,2,1]), X_old)
        X_new=tf.reshape(X_new,[batch,N_new, 1,C])
        
        A_new = tf.matmul(A_g,S)
        A_new = tf.matmul(tf.transpose(S,[0,2,1]), A_new)
        return X_new,A_new,S

    def gcnPooling_v11(self, X_g, A_g, scope, pooling_ratio=0.8):
        batch, N, M, C = X_g.get_shape()
        N_new = int((int(N) * pooling_ratio))        
        ...
        attW_r = self._definedTuckerWeight_variable('diagParam', [N_new], scope, regularization=False)
        diagParam=tf.matrix_diag(attW_r)
        zeros=tf.zeros([int(N-N_new), N_new])
        diagParam=tf.concat([diagParam,zeros],axis=0)
        diagParam=tf.reshape(diagParam,[1,N,N_new])
        SVD_diag = tf.tile(diagParam, multiples=[batch,1,1])

        p_score = tf.matmul(A_g, X_g)
        p_score_T = tf.transpose(p_score, [0, 2, 1])
        SVD_l = tf.nn.leaky_relu(tf.matmul(p_score, p_score_T),alpha=0.25) 
        
        a = self._definedTuckerWeight_variable('attentionA', [1, N_new, N_new], scope, regularization=False)
        SVD_r = tf.tile(a, multiples=[batch, 1, 1])
                
        svd_res=tf.matmul(SVD_l, SVD_diag)
        svd_res=tf.matmul(svd_res,SVD_r)
        S=tf.nn.softmax(svd_res, dim=2)
    ...
    def gcnPooling_v12(self, X_g, A_g, scope, pooling_ratio=0.8):
        batch, N, M, C = X_g.get_shape()
        N_new = int((int(N) * pooling_ratio))        
        ...
        diag_r = self._definedTuckerWeight_variable('diagParam', [N_new], scope, regularization=False)
        diagParam=tf.matrix_diag(diag_r)
        zeros=tf.zeros([int(N-N_new), N_new])
        diagParam=tf.concat([diagParam,zeros],axis=0)
        diagParam=tf.reshape(diagParam,[1,N,N_new])
        SVD_diag = tf.tile(diagParam, multiples=[batch,1,1])

        conv_x = tf.matmul(X_g, tf.transpose(X_g, [0, 2, 1]))
        SVD_l = tf.nn.leaky_relu(conv_x,alpha=0.25) 
        ...
        attW_r = self._definedTuckerWeight_variable('attentionW_r', [1, N, N_new], scope, regularization=False)
        attWs_r = tf.tile(attW_r, multiples=[batch, 1, 1])
        res_r = tf.matmul(p_score_T,attWs_r)
        res_l = tf.identity(res_r)
        conv_r = tf.matmul(tf.transpose(res_l, [0, 2, 1]), res_r)
        SVD_r = tf.nn.leaky_relu(conv_r,alpha=0.25)
    ...
    def gcnPooling_V8(self, X_g, A_g, idx_g, scope, pooling_ratio=0.8):
        batch, N, M, C = X_g.get_shape()#25,384,60,1
      
        X_g = tf.reshape(X_g, [batch, N, M * C])#25,384,60
        X = tf.matmul(X_g, tf.transpose(X_g,[0,2,1]))#25,384,384
        W = self._definedTuckerWeight_variable('W', [1, N, N], scope, regularization=False)
        Ws = tf.tile(W, multiples=[batch, 1, 1])
        b = self._definedTuckerWeight_variable('b', [1, N, N], scope, regularization=False)
        bs = tf.tile(b, multiples=[batch, 1, 1])
        R=tf.matmul(W,tf.nn.relu(X))+bs#25,384,384
        S=tf.nn.softmax(R)+0.5#25,384,384
        merges=[]
        idx_pool=tf.argmax(S,axis=2)
        ...
        a = self._definedTuckerWeight_variable('attentionA', [1, N_new, N_new], scope, regularization=False)
        a_s = tf.tile(a, multiples=[batch, 1, 1])
        e = tf.nn.leaky_relu(tf.matmul(a_s, res), alpha=0.2)
        S_normalized = tf.nn.softmax(e, dim=1)
        X_new = tf.matmul(res_l, S_normalized)
        A_new = tf.matmul(A_g,attWs_r)
        A_new = tf.matmul(tf.transpose(attWs_r,[0,2,1]), A_new)

        return tf.transpose(X_new, [0, 2, 1]),A_new,idx_g
    ...
    def gonv_new(self, x, new_A, K, spatialKernel):
        batch, N, M, Cin = x.get_shape()
        x_inputlist_spatialLayer=tf.split(x,M,2)
        x_outputList_spatialLayer=[]
        for tempIdx in range(M):
            temp_input=tf.reshape(x_inputlist_spatialLayer[tempIdx],[batch, N, Cin])
            temp_output=self.chebyshev5ForBaseline(temp_input, new_A, 2, K, spatialKernel)
            x_outputList_spatialLayer.append(temp_output)
        x_output_spatialLayer=tf.stack(x_outputList_spatialLayer, axis=2)
        return x_output_spatialLayer
    ...
    def onehotLayer(self, x, region):
        batch, M = x.get_shape()
        x_floor = tf.floor(x)
        x_inside = (x - x_floor)*16
        x_inside=tf.expand_dims(x_inside,-1)
        x_inside=tf.tile(x_inside,[1,1,384])
        x = tf.to_int32(x_floor)
        x = tf.one_hot(x, region)

      
        print('..... x:', x)
        return tf.multiply(x,x_inside)

    def onehotLayer1(self, x, region):
        batch, M = x.get_shape()
        x_floor = tf.floor(x)
        x_inside = (x - x_floor)*16
        x_inside=tf.expand_dims(x_inside,-1)
    ...
    def _inference(self, x): 
        ...
        batch, M, Ci = x.get_shape()
        
        print('inital x:',x)
        inputs=tf.split(x,2,2)
        
        xInput=tf.to_int32(inputs[0])
        x=tf.one_hot(xInput, 384)
        print('.....x:',x)
        
        offsetInput=tf.reshape(inputs[1],[batch,M,1,1])
        offset=tf.tile(offsetInput,[1,1,1,384])
        offset=tf.to_float(offset)
      
        copyX=tf.identity(inputs[1])
        ...
        scope='scaleTime'
        temporal1Kernel=self._definedTuckerWeight_variable('xInput1Kernel', [1, 3, 1, 2], scope, regularization=False)
        ...
        x = tf.nn.max_pool(x,[1,1,3,1],[1,1,2,1],padding='VALID')
        print('x:',x)        
        temporal2Kernel=self._definedTuckerWeight_variable('xInput2Kernel', [1, 3, 2, 4], scope, regularization=False)
        x=tf.nn.conv2d(x, 
                        temporal2Kernel,
                        strides=[1,1,1,1], 
                        padding='SAME')
        ...
        temporal3Kernel=self._definedTuckerWeight_variable('xInput3Kernel', [1, 3, 4, 8], scope, regularization=False)
        x=tf.nn.conv2d(x, 
                        temporal3Kernel,
                        strides=[1,1,1,1], 
                        padding='SAME')
        ...
        x = tf.nn.batch_normalization(x, x_mean, x_varia, offset, scale, vari_epsl)
        ...
        temporal4Kernel=self._definedTuckerWeight_variable('xInput4Kernel', [1, 3, 8, 4], scope, regularization=False)
        x=tf.nn.conv2d(x, 
                        temporal4Kernel,
                        strides=[1,1,1,1], 
                        padding='SAME')
        ...
        temporal5Kernel=self._definedTuckerWeight_variable('xInput5Kernel', [1, 3, 4, 2], scope, regularization=False)
        x=tf.nn.conv2d(x, 
                        temporal5Kernel,
                        strides=[1,1,1,1], 
                        padding='SAME')
        x=tf.nn.relu(x)
        print('x:',x)
        x0 = tf.nn.max_pool(x,[1,1,3,1],[1,1,2,1],padding='VALID')
        ...
        print('x0:',x0)
        x0 = self.fc(x0, 60, 'timeFC', relu=False)
        print('timeFC x0:',x0)#(4800, 60)
      
        N,M= x0.get_shape()
      
        [x_mean0, x_varia0] = tf.nn.moments(x0, axes=[0])
        x0 = tf.nn.batch_normalization(x0, x_mean0, x_varia0, offset, scale, vari_epsl)
        print('batch_norm x0:', x0)
        N,M=x0.get_shape()
        x0=tf.reshape(x0,[batch, N//batch, 1, 60])
        ...
        K=3
        A_diag0=self.redefine_A_6(x0, new_A, 'param1')
        spatialKernel1 = self._definedTuckerWeight_variable('gcnKernel1',[K, 60, 60], ('encodeDecoderConv{}').format(1), regularization=False)
        x1=self.gonv_new(x0, A_diag0, K, spatialKernel1)
        x1,A_diag1= self.gcnPooling_V14(x1, A_diag0, ('gcnPooling{}').format(1), 0.6)

      
        [x_mean1, x_varia1] = tf.nn.moments(x1, axes=[0,1,2])
        x1 = tf.nn.batch_normalization(x1, x_mean1, x_varia1, offset, scale, vari_epsl)
        x1=tf.nn.relu(x1)
        
        print('2 gcnconv',x1)
      
        A_diag2=self.redefine_A_6(x1, A_diag1, 'param2')
      
        spatialKernel2 = self._definedTuckerWeight_variable('gcnKernel2',[K, 60, 60], ('encodeDecoderConv{}').format(2), regularization=False)
        x2=self.gonv_new(x1, A_diag2, K, spatialKernel2)
        x2, A_diag2=self.gcnPooling_V14(x2, A_diag2, ('gcnPooling{}').format(2), 0.6)
        
        [x_mean2, x_varia2] = tf.nn.moments(x2, axes=[0,1,2])
        x2 = tf.nn.batch_normalization(x2, x_mean2, x_varia2, offset, scale, vari_epsl)
        x2=tf.nn.relu(x2)
        
        print('3 gcnconv',x2)
        A_diag3 = self.redefine_A_6(x2, A_diag2, 'param3')
      
        spatialKernel3 = self._definedTuckerWeight_variable('gcnKernel3',[K, 60, 60], ('encodeDecoderConv{}').format(3), regularization=False)
        x3=self.gonv_new(x2, A_diag3, K, spatialKernel3)
        x3, A_diag3=self.gcnPooling_V14(x3, A_diag3, ('gcnPooling{}').format(3), 0.6)

        [x_mean3, x_varia3] = tf.nn.moments(x3, axes=[0,1,2])
        x3 = tf.nn.batch_normalization(x3, x_mean3, x_varia3, offset, scale, vari_epsl)
        x3=tf.nn.relu(x3)
        
        print('4 gcnconv',x3)
      
        A_diag4=self.redefine_A_6(x3, A_diag3, 'param4')
      
        spatialKernel4 = self._definedTuckerWeight_variable('gcnKernel4',[K, 60, 60], ('encodeDecoderConv{}').format(4), regularization=False)
        x4=self.gonv_new(x3, A_diag4, K, spatialKernel4)
        x4, A_diag4=self.gcnPooling_V14(x4, A_diag4, ('gcnPooling{}').format(4), 0.6)
        
        [x_mean4, x_varia4] = tf.nn.moments(x4, axes=[0,1,2])
        x4 = tf.nn.batch_normalization(x4, x_mean4, x_varia4, offset, scale, vari_epsl)
        x4=tf.nn.relu(x4)
        
        print('5 gcnconv',x4)
        A_diag5 = self.redefine_A_6(x4, A_diag4, 'param5')
      
        spatialKernel5 = self._definedTuckerWeight_variable('gcnKernel5',[K, 60, 60], ('encodeDecoderConv{}').format(5), regularization=False)
        x5=self.gonv_new(x4, A_diag5, K, spatialKernel5)
        x5, A_diag5=self.gcnPooling_V14(x5, A_diag5, ('gcnPooling{}').format(5), 0.6)

        [x_mean5, x_varia5] = tf.nn.moments(x5, axes=[0,1,2])
        x5 = tf.nn.batch_normalization(x5, x_mean5, x_varia5, offset, scale, vari_epsl)
        x5=tf.nn.relu(x5)
        
        print('6 gcnconv',x5)
        A_diag6 = self.redefine_A_6(x5, A_diag5, 'param6')
      
        spatialKernel6 = self._definedTuckerWeight_variable('gcnKernel6',[K, 60, 60], ('encodeDecoderConv{}').format(6), regularization=False)
        x6=self.gonv_new(x5, A_diag6, K, spatialKernel6)
        x6, A_diag6=self.gcnPooling_V14(x6, A_diag6, ('gcnPooling{}').format(6), 0.6)

        [x_mean6, x_varia6] = tf.nn.moments(x6, axes=[0,1,2])
        x6 = tf.nn.batch_normalization(x6, x_mean6, x_varia6, offset, scale, vari_epsl)
        x6=tf.nn.relu(x6)
        ...
        x0_0 = tf.reduce_mean(x0, axis=1, keep_dims=True)
        x1_0 = tf.reduce_mean(x1, axis=1, keep_dims=True)
        x2_0 = tf.reduce_mean(x2, axis=1, keep_dims=True)
        x3_0 = tf.reduce_mean(x3, axis=1, keep_dims=True)
        x4_0 = tf.reduce_mean(x4, axis=1, keep_dims=True)
        x5_0 = tf.reduce_mean(x5, axis=1, keep_dims=True)
        x6_0 = tf.reduce_mean(x6, axis=1, keep_dims=True)
        
        x0_1 = tf.reduce_max(x0, axis=1, keep_dims=True)
        x1_1 = tf.reduce_max(x1, axis=1, keep_dims=True)
        x2_1 = tf.reduce_max(x2, axis=1, keep_dims=True)
        x3_1 = tf.reduce_max(x3, axis=1, keep_dims=True)
        x4_1 = tf.reduce_max(x4, axis=1, keep_dims=True)
        x5_1 = tf.reduce_max(x5, axis=1, keep_dims=True)
        x6_1 = tf.reduce_max(x6, axis=1, keep_dims=True)
        
        r0=tf.concat([x0_0,x0_1],axis=3)
        r1=tf.concat([x1_0,x1_1],axis=3)
        r2=tf.concat([x2_0,x2_1],axis=3)
        r3=tf.concat([x3_0,x3_1],axis=3)
        r4=tf.concat([x4_0,x4_1],axis=3)
        r5=tf.concat([x5_0,x5_1],axis=3) 
        r6=tf.concat([x6_0,x6_1],axis=3)
        ...
        batch, N, M,C = s0.get_shape()
        x=tf.reshape(s0,[batch, N*M*C])

        with tf.variable_scope('fc'):
          
          
          
            x = self.fc(x, 13, 'fc2', relu=False) 
            x = tf.nn.softmax(x)
        return x,x,copyX
